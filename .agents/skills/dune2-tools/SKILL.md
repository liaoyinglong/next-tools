---
name: dune2-tools
description: Use @dune2/tools effectively in React and TypeScript projects. Activate when a project depends on @dune2/tools or when implementing API requests/TanStack Query flows, local or session storage, cookie storage, Valtio state, React Context factories, prop adapters, typed field names, or precise number formatting/calculation. Prefer existing @dune2/tools APIs over reimplementing equivalent helpers.
license: ISC
metadata:
  package: '@dune2/tools'
...
Use this skill to reuse the library's existing abstractions instead of rebuilding them with raw TanStack Query, storage listeners, Valtio boilerplate, Context boilerplate, prop wrapper components, string field constants, or floating-point helpers.

## First checks

1. Confirm the project depends on `@dune2/tools` in its package manifest or lockfile.
2. Search the codebase for existing `@dune2/tools/*` imports and generated API modules before adding a new abstraction.
...
4. Match the project's existing request client, query client, namespaces, store conventions, and generated API patterns instead of introducing parallel infrastructure.
...
| Imperative request using an existing API definition | `api.request(...)` or `api.requestWithConfig(...)` |
| Prefetch/fetch/ensure/invalidate/refetch/read/write query cache | The matching `RequestBuilder` method |
| GET-style URL params plus a request body or other unusual transport config | `requestWithConfig(...)` |
...
- Mixed query/body/custom headers/signal/request function: `requestWithConfig`.
- URL placeholders: define `urlPathParams`; do not manually duplicate path replacement at call sites.
- Configure a shared transport with `RequestBuilder.setRequestFn(...)` when the project already follows that model. Avoid assigning a one-off request function to every API without a reason.
- In SSR, do not share one request-global `QueryClient` across users. Reuse the project's `RequestBuilder.setQueryClientFactory(...)` pattern when present.
...
- For cookie-backed client-readable strings, use `createCookieStorage` if the existing project uses it. Do not treat it as a replacement for server-set `HttpOnly` authentication cookies.
...
- `store.useSnapshot()` returns Valtio proxy-backed snapshots. When passing selected object/array state to third-party or child components, prefer the project's `useShallowSnapshot(...)` pattern or convert with `snapshot(...)` when needed.
...
1. Identify the nearest existing `@dune2/tools` pattern in the repository.
...
5. Run the repository's relevant typecheck/tests after changes.
