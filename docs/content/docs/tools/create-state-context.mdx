...
- `useContextValue: () => T` - 获取 Context 值的 Hook（内部使用 `useContext`）
- `Provider: FC<PropsWithChildren<P>>` - Provider 组件
- `withProvider: <C extends object>(Comp: ComponentType<C>, providerProps?: P) => FC<C>` - 高阶组件
- `Context: React.Context<T>` - 原始 Context 对象
- `use: () => T` - 使用 React 19 的 `use()` API 读取 Context 值（支持在异步组件中使用）
