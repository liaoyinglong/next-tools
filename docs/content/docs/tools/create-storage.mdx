...
- `namespace`: 命名空间，用于隔离不同模块的存储，会作为存储键名的前缀（如 `"myApp"` → 键名为 `"myApp.token"`）。不传或传空字符串时，直接使用原始属性名作为键名，适用于兼容老项目已有的 storage key
- `storageType`: 存储类型
...
完整的存储键名。设置了 namespace 时包含前缀，未设置时为原始属性名。
...
// 无 namespace
console.log(storage.token.key); // "token"
```
...
### 动态 key（createStorageHelper）

`createStorage` 要求在定义时就确定所有 key。如果你需要在运行时才确定 key（比如兼容老项目的动态 key），可以使用 `createStorageHelper` 来创建单个存储项：

```tsx
import { createStorageHelper } from '@dune2/tools/storage';

// 基本用法
const tokenHelper = createStorageHelper('token', '');
tokenHelper.set('abc');
tokenHelper.get(); // 'abc'

// 运行时拼接 key
function getUserToken(userId: string) {
  return createStorageHelper(`token_${userId}`, '');
}
const helper = getUserToken('123');
helper.key; // "token_123"

// 使用 sessionStorage
const cache = createStorageHelper('pageCache', null, 'session');
```

`createStorageHelper` 返回的实例和 `createStorage` 中的每个属性完全一致，支持 `get`、`set`、`remove`、`useValue`、`subscribe` 等全部方法。

如果同一组 key 结构相同、只是前缀不同，也可以用 `createStorage` 配合动态 `namespace`：

```tsx
function createUserPrefs(userId: string) {
...
const prefsA = createUserPrefs('alice');
prefsA.theme.set('dark'); // key: "user_alice.theme"
```

> **注意**：在 React 组件中使用 `useValue` 时，需要保证 `StorageHelper` 实例的引用稳定（在组件外创建，或用 `useRef` 持有），否则每次渲染都会创建新实例导致 `useValue` 失效。
...
### 3. 兼容老项目已有的 storage key

如果你正在迁移一个老项目，localStorage 中已有不带前缀的 key（如 `"token"`、`"userInfo"`），可以省略 `namespace` 来直接复用：

```tsx
// 老项目中已有的 key: "token", "userInfo"
const legacyStorage = createStorage({
  DataMap: LegacyDataMap,
  // 不传 namespace，key 直接就是属性名
});

console.log(legacyStorage.token.key); // "token" — 兼容老数据
```
