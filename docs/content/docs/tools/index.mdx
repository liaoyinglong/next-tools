...
如果你使用 Coding Agent 开发依赖 `@dune2/tools` 的项目，可以安装项目提供的 consumer Agent Skill。它会帮助 Agent 主动发现库里已有但容易被忽略的 API，并按 `rq / storage / store / factory / numbro` 任务领域只加载相关参考内容。

```bash
npx skills add liaoyinglong/next-tools --skill dune2-tools
```

详见：[Agent Skill](./agent-skill)
