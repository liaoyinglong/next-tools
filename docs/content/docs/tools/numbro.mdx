...
- 🔗 **链式调用**: 所有运算方法返回新 Numbro 实例，支持链式操作
...
`Numbro` 本身不提供 `formatCurrency` 方法，但可以通过 `format()` 的 `prefix`/`postfix` 配置实现货币风格展示：

```tsx
// 美元格式
numbro(1234.56).format({
  prefix: '$',
...
```tsx
// 访问底层 BigNumber 构造函数
Numbro.rawBigNumber; // BigNumber 类

// 访问 RoundingMode 枚举
Numbro.RoundingMode;

// 预配置的 BigNumber clone（默认 ROUND_DOWN）
Numbro.BN;
```
...
num.gt(50); // true  — 大于
num.gte(100); // true  — 大于等于
num.lt(200); // true  — 小于
num.lte(100); // true  — 小于等于
num.eq(100); // true  — 等于
```
