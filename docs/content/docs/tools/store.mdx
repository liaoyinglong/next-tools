...
创建的 store 实例包含以下属性和方法：

- `state` — 可变 Proxy 对象，直接写入即触发更新
- `initialState` — 初始状态的快照（只读）
- `getState()` — 获取当前状态快照
- `actions` — 由 `actionsCreator` 返回的操作方法
- `subscribe(fn)` — 订阅状态变化，返回取消函数
- `useSnapshot()` — React Hook，基于 valtio 的 Proxy 追踪
- `useShallowSnapshot(selector)` — React Hook，通过 selector 精确选择 + 深比较
...
// 获取初始状态
const initial = store.initialState;
...
### 注意：Proxy 对象不能直接传递给子组件

`useSnapshot` 返回的是 Proxy 对象。如果你从中取出 object 或 array 类型的值并传递给其他组件，可能会导致意想不到的错误（如子组件内部对该值做序列化、比较等操作时出现异常）。

有两种解决方案：

**方案一：使用 `useShallowSnapshot`（推荐）**

`useShallowSnapshot` 返回的是普通值，可以安全传递：

```typescript
function Parent() {
  const { user } = store.useShallowSnapshot((s) => ({ user: s.user }));
...
如果仍想使用 `useSnapshot`，在传递给子组件前用 `snapshot` 把 Proxy 对象转换为普通对象：

```typescript
import { snapshot } from '@dune2/tools/store';
