...
Use this reference when choosing an existing `@dune2/tools` abstraction in a consumer application. Prefer application-local examples and generated API definitions when they differ from these generic examples.
...
### requestWithConfig: escape hatch

`requestWithConfig(...)` is generally **not** the preferred calling style. Default to `request(params)` because it preserves the endpoint's normal parameter conventions and keeps call sites simple.

Use `requestWithConfig(...)` only when a real endpoint requirement cannot be expressed cleanly through `request(...)`, for example when the same request genuinely needs separately controlled query params and body data, or a per-call request configuration override.
...
Do not introduce it merely to pass ordinary GET params or ordinary mutation data. Follow existing application usage when an endpoint already has an established pattern.

### Shared request function

An application can configure transport once:
...
Use `storageType: 'session'` for sessionStorage. An empty/omitted namespace changes the key naming behavior, so preserve the application's compatibility requirements.
...
Prefer actions for mutations unless the application intentionally uses direct `store.state` writes.
...
The wrapper is based on BigNumber.js and supports chainable precise operations and configurable rounding. Search existing application code for project-specific formatting options before inventing new display rules.
